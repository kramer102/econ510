...
## gave me all the column names and types
test <- dbGetQuery(db, 'PRAGMA table_info(Scorecard)')
...
SELECT s11.INSTNM College,
                       ...
                       WHERE s11.Year=2011
                       AND s13.Year=2013
                       AND s11.SAT_AVG_ALL IS NOT NULL
...
m1 <- lm(e90 ~ sat,data=earnings)
summary(m1)
qplot(sat,e90,data = earnings)
### End Example
